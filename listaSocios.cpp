...
void listaSocios::agregarSocio(const constancia &socio)
{
    nodo *temp = ancla;
    while (temp != nullptr)
    {
        if (temp->getDato().getNombre() == socio.getNombre())
        {
...
void listaSocios::enqueu(const constancia &cons)
{
    agregarSocio(cons);
}


void listaSocios::dequeu()
{
    if (ancla == nullptr) {
...
bool listaSocios::vacia()
{
    return (frontal == nullptr);
}

void listaSocios::llena()
{
    if (ancla != nullptr) {
        constancia primeraConstancia = ancla->getDato();

        cout << "Llena la constancia del alumno < " << primeraConstancia.getNombre() << " >" << endl;
        primeraConstancia.llenar();
    }
}
